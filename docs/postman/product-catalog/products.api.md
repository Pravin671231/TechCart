...
**Scope:** this document covers what's implemented as of Issue #31 (M2.7 — Product core CRUD and pricing, `FR-CAT-001`–`013`, `FR-CAT-085`–`087`), Issue #32 (M2.8 — Product variants, `FR-CAT-039`–`044`), Issue #33 (M2.9 — Status update APIs, `FR-CAT-045`, `048`–`049` for this entity), Issue #34 (M2.10 — Admin search, `FR-CAT-050`, `053` for this entity), Issue #35 (M2.11 — Buyer browsing, search & inventory visibility, `FR-CAT-054`–`067`, `095`–`096`), Issue #36 (M2.12 — Buyer filtering, sorting & card content, `FR-CAT-068`–`076`, `091`–`092`), Issue #102 (SRS v0.2 amendment — variant-only pricing, stock/inventory tracking removed system-wide), and Issue #189 (SRS v0.10/M10.1 — buyer `availability` and `?inStock=true` reinstated, inventory-backed): the four product-level admin CRUD endpoints under `/api/admin/products` (create/update/get/list — there is no product-level stock-update path anymore), the status-transition path, the two embedded-variant endpoints, `search`/`status` filtering on the admin list, and the two buyer-facing endpoints — `GET /api/products` (paginated, `published`-only, filterable, sortable, keyword `?q=`) and `GET /api/products/:slug` (detail). _(Issue #322: a standalone `?q=` keyword search is a plain MongoDB regex over `name`/`description` and works against any MongoDB. Only the variant-attribute/specification filters — and `?q=` combined with one of them — need a MongoDB **Atlas** cluster plus a one-time `npm run search:ensure --workspace backend` to build the `products_search` index; run against anything else they return `503 SEARCH_UNAVAILABLE`. See [Testing the Atlas Search filters](#testing-the-atlas-search-filters) below.)_ Every other query on this endpoint (category/brand/price/on-sale/sort/`inStock`) needs no Atlas cluster at all. See [`uploads.api.md`](./uploads.api.md) for `GET /health`, the R2 upload endpoints, and the one-time Postman collection setup; see [`brands.api.md`](./brands.api.md) and [`categories.api.md`](./categories.api.md) for the two entities every product references (including categories.api.md's own `GET /api/categories/:slug/products`, which lists *this* module's data and shares this endpoint's filter/sort surface, `inStock`/`availability` included); see [`categorySpecifications.api.md`](./categorySpecifications.api.md) for the schema a product's `specifications` — and the `filterable` flag driving both the specification filter and `cardSpecifications` below — are validated against; see [`../inventory/inventory.api.md`](../inventory/inventory.api.md) for the per-warehouse stock rows `availability`/`inStock` are actually computed from. This doc assumes collection setup is already done and reuses the same collection.
...
**Issue #189/M10.1 in one paragraph:** stock/inventory tracking and the buyer-facing `availability` field, both removed entirely by #102, are back — but backed by the new `inventory` module ([`../inventory/inventory.api.md`](../inventory/inventory.api.md)), not by a field on the product or variant itself. Stock is tracked per `(variant, warehouse)`; `availability` is always a derived 2-state summary (`in_stock`/`out_of_stock`) computed from that inventory data at read time — never a raw count, and never anything the buyer surface exposes per-warehouse. See [`GET /api/products`](#get-apiproducts) and [`GET /api/products/:slug`](#get-apiproductsslug) below for exactly where it appears.
...
Lists **published-only** products, paginated (`FR-CAT-054`). The first buyer-facing endpoint in this API — no auth header, and every response strips `status`/`createdBy`/`updatedBy` (`FR-CAT-095`). There is still no `stock`/`lowStockThreshold` field anywhere in this system (Issue #102 — every catalog entry is treated as orderable from a pure catalog standpoint), but a derived `availability` field **is** present again as of Issue #189/M10.1, computed from the inventory module — see below.
...
| `sort`                                    | `relevance` (only meaningful with `q`) \| `price_asc` \| `price_desc` \| `newest` (`FR-CAT-075`) \| `recommended` (flat listing only, `FR-CAT-105`) | see below |
| `inStock`                                 | the literal string `true` (presence-based) — narrows to products with at least one active variant carrying stock > 0 across active warehouses (Issue #189/M10.1)     | omitted   |

_Amended, Issue #102, then reinstated Issue #189/M10.1: `inStock` was removed system-wide by #102 (no stock concept at all) and is back as of #189, now backed by the `inventory` module — see [`../inventory/inventory.api.md`](../inventory/inventory.api.md). It's a plain-query filter (an `$elemMatch` alongside the existing price/on-sale filters) and needs no Atlas cluster, same as `minPrice`/`maxPrice`/`onSale`._

`sort`'s default: `relevance` when `q` is present, `newest` otherwise. A standalone `q` runs as a plain regex and works with no Atlas cluster (Issue #322 — the `relevance` sort degrades to `newest`, since a regex has no score).

`sort=recommended` (`FR-CAT-105`, SRS v0.2 amendment) is the ordering the `buyer-app` home page sends by default. It interleaves published products **across their categories** — rank every product within its own category by newest-primary-variant-first, then emit rank 1 of each category, then rank 2 of each, and so on — so the grid always looks varied instead of one category's whole run before the next. It is only acted on by the plain flat listing: a `?category=` scope, the dedicated `GET /api/categories/:slug/products` route, or a `q` all fall back to `newest`. Like every other sort it applies a total `_id` tie-breaker (`FR-CAT-106`), so incremental-scroll pages never overlap or drop a product and a refetch does not reshuffle. `attributeName`/`attributeValue` and any `spec[...]` filter route the query through the `products_search` Atlas Search index even with no `q` at all — and a `q` combined with one of those also goes through Atlas. That index needs an Atlas cluster and `npm run search:ensure` (a one-time build) — otherwise these requests return `503 SEARCH_UNAVAILABLE`; see [Testing the Atlas Search filters](#testing-the-atlas-search-filters) below. `minPrice`/`maxPrice`/`onSale` are plain-query filters and work with no Atlas cluster, same as before.
...
- **`availability`** (Issue #189/M10.1) — a top-level, per-**product** field on this list item, one of `"in_stock"`/`"out_of_stock"`: `"in_stock"` when at least one active variant has stock > 0 summed across every warehouse; `"out_of_stock"` when every active variant sums to `0`. **Absent entirely (not `null`)** when the product has zero active variants — the same documented edge case that also drops `mrp`/`discount`/`sellingPrice`/`primaryImage` below. Never exposes which warehouse or how much stock — just the 2-state summary.
...
- **`cardSpecifications`** (`FR-CAT-091`–`092`) is a category's first six `filterable` specification fields, in schema declaration order, always present — `[]` on a home/all-products listing (no shared category) or a category with no filterable fields defined. A field the product has no stored value for is skipped, not padded — a product with only two of a category's six filterable fields shows exactly two. `unit` is `null` (not omitted) when the field has none.
