...
This index covers M2 (Product Catalog) only — Issues #25 through #36. No real authentication exists yet (v0.3): every admin endpoint below is guarded by a placeholder `X-Admin-Key` header, not a session/RBAC system. No other domain (Orders, etc.) has a doc here yet.
...
| 6 | [`products.api.md`](./product-catalog/products.api.md) | Product CRUD, pricing, embedded variants, status/stock, admin search, buyer browsing/filtering/sorting | #31, #32, #33, #34, #35, #36 |
...
| PATCH | `/api/admin/products/:id/stock` | admin | [→](./product-catalog/products.api.md#patch-apiadminproductsidstock) |
...
**36 endpoints total** across the 6 files above (3 + 7 + 9 + 3 + 3 + 11 — `categories.api.md`'s two pointer tables aren't counted separately, since their real rows are already listed under `categorySpecifications.api.md`/`categoryVariants.api.md`).
