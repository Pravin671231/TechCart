...
- Material React Table (MUI-based) for catalog/order grids — built-in filter, sorting, pagination, column visibility (toolbar-level), and loading (spinner/skeleton) states, rather than hand-building each on top of headless TanStack Table. Row selection and the per-column actions menu ship with the library but are turned off (`enableRowSelection: false`, `enableColumnActions: false`) — not part of this app's UI. MUI's theme stays scoped to the table component only (`admin-app/src/components/DataTable.tsx`, custom-themed to match the Tailwind design tokens in `admin-app/docs/design-tokens.md`) — it is not applied app-wide, so it can't collide with the rest of `admin-app`'s Tailwind-only chrome.
