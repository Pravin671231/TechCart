...
export const Footer = ({ onNavigate, variant = "rail" }: FooterProps) => {
  ...
  return (
    <div className="mt-auto border-t border-neutral-200 p-2 lg:p-3 dark:border-neutral-800">
      <div className={cn("flex items-center gap-2 rounded-md p-2", isRail && "flex-col lg:flex-row")}>
        ...
        <div className={cn("min-w-0 flex-1", isRail && "hidden lg:block")}>
          <Link
            to="/account"
            onClick={onNavigate}
            className="block truncate text-sm font-medium text-neutral-900 hover:text-primary-600 dark:text-neutral-100"
          >
            {displayName}
          </Link>
        </div>
