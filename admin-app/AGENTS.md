...
- `src/features/<feature>/` holds the actual UI/logic for that feature. `src/features/dashboard/Dashboard.tsx` (rendered at `/`) is the first real one. `src/features/product-catalog/{products,categories,brands,specifications,variant-types}/` are placeholder pages wired into real routes and the `Sidebar` nav, following the same "coming soon" idiom until each gets its real implementation — grouped under `product-catalog/` because all five belong to SRS v0.2 (`docs/srs/features/0.2-product-catalog.md`), unlike `dashboard/` which doesn't.
