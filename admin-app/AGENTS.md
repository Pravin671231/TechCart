...
- `src/features/<feature>/` holds the actual UI/logic for that feature. `src/features/dashboard/Dashboard.tsx` (rendered at `/`) is the first real one; `products/`, `categories/`, `brands/`, `specifications/`, `variant-types/` are placeholder pages wired into real routes and the `Sidebar` nav, following the same "coming soon" idiom until each gets its real implementation.
