...
**Sticky header, bounded height.** `DataTable.tsx` sets `enableStickyHeader: true` plus `muiTableContainerProps: { sx: { maxHeight: "600px" } }`, so a long table scrolls its body internally with the header pinned, instead of growing the page. It also overrides `renderToolbarInternalActions` to render only `MRT_ToggleFullScreenButton` — MRT's default "Show/Hide columns" toolbar button (and its column-visibility menu) is omitted entirely, by choice, not just per-column.
