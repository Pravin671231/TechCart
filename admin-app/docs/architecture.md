...
├── layout/
│   ├── AdminShell.tsx          # composes Sidebar + Header + MainSection; owns isSidebarOpen state
│   ├── Sidebar.tsx                # dark nav — fixed 240px on lg+, off-canvas drawer below lg
│   ├── Header.tsx                 # top bar — hamburger (lg:hidden), search, notification/theme icons, user info
...
**Responsive shell.** Below `lg` (1024px), `Sidebar` is an off-canvas drawer, not part of the flex row — `fixed`, hidden by `-translate-x-full` until opened. `AdminShell.tsx` owns the `isSidebarOpen` state (the nearest common parent of `Sidebar` and `Header`) and passes `isOpen`/`onClose` to `Sidebar`, `onToggleSidebar` to `Header`, whose hamburger button triggers it (and is itself `lg:hidden`, since the sidebar is permanently visible at `lg`+ and there's nothing to toggle). The drawer closes via a backdrop click, an in-drawer `✕` button, or Escape (`Sidebar`'s own `keydown` listener, added only while open). See `docs/design-tokens.md`'s Layout section for the full breakpoint scale and per-component responsive behavior (`PageHeader`, `Dashboard`'s stat grid, `MainSection` padding).
...
│   ├── shell.test.tsx                        # AdminShell sidebar/header/content, mobile drawer open/close (backdrop, close button, Escape)
