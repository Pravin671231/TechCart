...
├── layout/
│   ├── AdminShell.tsx          # composes Sidebar + MobileMenuBar + MainSection; owns isSidebarOpen state
│   ├── Sidebar.tsx                # dark nav — fixed 240px on lg+, off-canvas drawer below lg; profile block pinned at the bottom
│   ├── MobileMenuBar.tsx          # lg:hidden top bar — just the hamburger button that opens the sidebar drawer
...
│   ├── StatusSelect.tsx           # row-wise editable status dropdown (MUI Select), used inside a Status column's Cell
│   └── StatusBadge.tsx            # { label, tone } read-only pill, tone → success/warning/danger/neutral tokens
├── features/
...
│       │   ├── ProductsPage.tsx                        # low-stock toggle + DataTable, stateful mock data (editable status)
...
**Responsive shell.** Below `lg` (1024px), `Sidebar` is an off-canvas drawer, not part of the flex row — `fixed`, hidden by `-translate-x-full` until opened. `AdminShell.tsx` owns the `isSidebarOpen` state (the nearest common parent of `Sidebar` and `MobileMenuBar`) and passes `isOpen`/`onClose` to `Sidebar`, `onToggleSidebar` to `MobileMenuBar`, whose one button triggers it. `MobileMenuBar` itself is `lg:hidden` — at `lg`+ the sidebar is permanently visible and there's nothing to toggle, so nothing renders in that slot at all (not just visually hidden — it takes no layout space, so `AdminShell`'s `flex-col gap-2` doesn't leave a phantom gap for it). The drawer closes via a backdrop click, an in-drawer `✕` button, or Escape (`Sidebar`'s own `keydown` listener, added only while open). There used to be a fuller `Header.tsx` here (search, theme/fullscreen toggles, notifications, profile) — all of that was removed except the menu button, which `MobileMenuBar` kept; the profile block moved into `Sidebar.tsx` instead (see below), not replaced. See `docs/design-tokens.md`'s Layout section for the full breakpoint scale and per-component responsive behavior (`PageHeader`, `Dashboard`'s stat grid, `MainSection` padding).

**Profile block lives at the bottom of `Sidebar.tsx`.** `<nav>` is `flex-1`, so it grows to fill the sidebar's available height, pushing the profile block (avatar + name/role) that follows it down to the bottom edge — present in both the `lg`+ static column and the Mobile/Tablet drawer, since it's genuinely part of the sidebar now rather than a separate top bar.

`ProductsPage.tsx`, `CategoriesPage.tsx`, and `BrandsPage.tsx` are the placeholders promoted to real (mock-data) content — per root `docs/architecture.md` §4.2's tech blueprint, all three render mock data through the shared `DataTable`, backed by **Material React Table** (search, per-column filters, sorting, pagination, and toolbar-level column visibility all built in, rather than hand-built — see `docs/design-tokens.md`'s design-tokens note and `muiTheme.ts`). MRT also ships row-selection checkboxes and a per-column "⋮" actions menu; both are explicitly disabled (`enableRowSelection: false`, `enableColumnActions: false` in `DataTable.tsx`) — not part of this app's UI. Each composes the shared `PageHeader` rather than `MainSection.tsx` owning that chrome directly, because `MainSection` wraps every route including `Dashboard`, whose header shape (stat cards) is entirely different — `PageHeader` is the reusable piece, used per-page, not baked into the universal wrapper. `DataTable`/`StatusBadge`/`StatusSelect` live in `src/components/`, not `src/layout/`, because they're generic reusable widgets, not page-layout chrome — same distinction as keeping `AdminShell`/`Sidebar` out of `src/features/`. There's no backend list endpoint wired into `admin-app` yet (that's a separate TanStack Query integration) — each page simulates a brief loading delay (`isLoading` state, ~500ms `setTimeout` on mount) so `DataTable`'s skeleton/spinner states are genuinely exercised rather than left as unused props. Products additionally keeps one small piece of custom state — a "low stock only" checkbox — since that's a derived cross-field filter (`stock <= lowStockThreshold`) MRT has no native column concept for; everything else (search, the Status column's filter, sorting, pagination, column visibility) is MRT's built-in toolbar. Specifications/Variant Types stay plain placeholders, and none of the three real pages include the inline create/edit form panel shown in their mocks — that's a separate, larger feature.

**Status is row-editable, not just displayed.** Each Status column's `Cell` renders `StatusSelect` (a small MUI `Select`) instead of a static `StatusBadge`, bound to that row's value and an `onChange` that updates it. Since the mock arrays would otherwise be static, each page holds its records in local `useState` (seeded from the `mockProducts`/`mockCategories`/`mockBrands` import) rather than using the import directly, so an edit actually persists in the table across re-renders. `StatusBadge` still exists for any future read-only status display (e.g. a detail page) — the two components serve different purposes.

**Alignment**: the Name column stays left-aligned (default, untouched — including Categories' subcategory-indent `Cell` renderer); every other column is explicitly `{ align: "center" }` on both `muiTableHeadCellProps` and `muiTableBodyCellProps`. Verified in MRT's source that centering, unlike right-alignment, does **not** reverse the header's flex direction — no sort-icon-flip issue to work around, so this is a plain object, no shared helper needed (the earlier `rightAlignedHeadCellProps`/`tableCellProps.ts` workaround was removed once nothing needed right-alignment anymore).

**Sticky header, bounded height.** `DataTable.tsx` sets `enableStickyHeader: true` plus `muiTableContainerProps: { sx: { maxHeight: "calc(100vh - 320px)" } }` — viewport-relative rather than a flat pixel value, so the table scrolls its body internally with the header pinned without overrunning the 768px-tall "minimum desktop" target viewport (see `docs/design-tokens.md`'s Layout section for the full supported-viewport table). It also overrides `renderToolbarInternalActions` to render only `MRT_ToggleFullScreenButton` — MRT's default "Show/Hide columns" toolbar button (and its column-visibility menu) is omitted entirely, by choice, not just per-column.

**MUI stays scoped to `DataTable.tsx`.** Material React Table pulls in `@mui/material`/`@mui/icons-material`/`@emotion/react`/`@emotion/styled` as peer dependencies — a second styling system alongside `admin-app`'s Tailwind-only setup everywhere else. `muiTheme.ts` is a custom MUI theme (primary color matched to the `#16A34A` design token, `shape.borderRadius: 8` matching `rounded-lg`) wrapped in a `<ThemeProvider>` **only** inside `DataTable.tsx` — no `CssBaseline`, no app-wide provider — so it can't leak into or reset `Sidebar`/`MobileMenuBar`/`PageHeader`, which stay pure Tailwind.
...
│   ├── shell.test.tsx                        # AdminShell sidebar (incl. profile)/menu bar/content, mobile drawer open/close (backdrop, close button, Escape)
│   ├── routes.test.tsx                        # clicks each sidebar nav link, asserts the matching page heading
│   ├── products.test.tsx                      # ProductsPage: renders after simulated load, low-stock toggle, status dropdown edits a row
│   ├── categories.test.tsx                    # CategoriesPage: renders mock rows, subcategory indentation, status dropdown edits a row
│   ├── brands.test.tsx                        # BrandsPage: renders mock rows, status dropdown edits a row
...
└── src/
    ...
    ├── layout/{AdminShell.tsx,Sidebar.tsx,MobileMenuBar.tsx,MainSection.tsx,PageHeader.tsx}
    ├── components/{DataTable.tsx,muiTheme.ts,StatusSelect.tsx,StatusBadge.tsx}
...
- **Icons**: `react-icons`, using its `lu` (Lucide) icon set exclusively (`react-icons/lu`) for a consistent line-icon style across `Sidebar`, `MobileMenuBar`, and `Dashboard`.
- **Tables**: `material-react-table` for catalog/order grids, per root `docs/architecture.md` §4.2 — chosen over hand-building on headless `@tanstack/react-table` because it ships filter, sort, pagination, sticky-header/bounded-height scrolling, and loading (spinner/skeleton) states out of the box (row selection, the per-column actions menu, and the "Show/Hide columns" toolbar button ship too but are disabled/omitted — see above). Wrapped once by `src/components/DataTable.tsx` rather than each page building its own `useMaterialReactTable` call. Its MUI peer dependencies (`@mui/material`, `@mui/icons-material`, `@emotion/react`, `@emotion/styled`) and `src/components/muiTheme.ts` are scoped to `DataTable.tsx` only — see the "MUI stays scoped" note above.
