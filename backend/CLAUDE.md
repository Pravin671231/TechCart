...
- `uploads` — `POST /api/admin/uploads/presign` (Issue #26 / M2.2). Owns the `presignedUploads` tracking collection but no product-like resource of its own; exports `consumeImageKeys`/`validateImageCount`/`normalizeImages` for reuse by later modules — see R2 uploads above.
- No brand/category/product modules exist yet. Issues #27–#36 mount their modules under `adminRouter` (admin-side) or the root router (buyer-facing) as they land.
