...
// This suite needs a real MongoDB instance — account-dedup logic
// (FR-AUTH-005/006) can't be trusted against a mocked repository, and the
// new session/OTP engines (#257/#258) are themselves Mongoose-model-backed.
...
describe("Buyer passwordless authentication", () => {
  describe("Google One Tap (POST /api/auth/one-tap/callback)", () => {
    it("creates a buyer account on first sign-in", async () => {
      mockGoogleIdentity({ email: "onetap-buyer@example.com", name: "One Tap Buyer" });
      ...
      expect(res.body.data.email).toBe("onetap-buyer@example.com");
    ...
    it("rejects one-tap sign-in for an email already registered as an admin", async () => {
      ...
      mockGoogleIdentity({ email: "admin-onetap@example.com", name: "Admin" });
  ...
  describe("Email OTP (send-verification-otp + sign-in/email-otp)", () => {
    it("creates a buyer account on first OTP sign-in", async () => {
      ...
      const record = await mongoose.connection.db!.collection("otps").findOne({ email });
      ...
      // every environment; carried over to the hand-rolled otp.ts engine.
    ...
    it("rejects an expired OTP", async () => {
      ...
      // expiry into the past instead of faking the system clock.
      await mongoose.connection
        .db!.collection("otps")
        .updateMany({ email }, { $set: { expiresAt: new Date(Date.now() - 1000) } });

      const res = await request(app).post("/api/auth/sign-in/email-otp").send({ email, otp });
      expect(res.status).toBeGreaterThanOrEqual(400);
      expect(res.body.success).toBe(false);
      // Distinct from the reused-OTP case's INVALID_OTP.
  ...
  describe("Cross-method account dedup (FR-AUTH-005)", () => {
    it("resolves a second buyer sign-in method with the same email to the original account", async () => {
      ...
      mockGoogleIdentity({ email, name: "Dedup Buyer" });
      const oneTapRes = await request(app)
        .post("/api/auth/one-tap/callback")
        .send({ idToken: "fake-id-token" });
      const firstUserId = oneTapRes.body.data.id;
      ...
      expect(otpRes.body.data.id).toBe(firstUserId);
  ...
  // (still applicable verbatim to the new engine, since #257's session.ts
  // is the same bearer-token-alongside-cookie design): buyer-app (Vercel)
  // and backend (Render) are cross-site, so a `fetch` there never reliably
  // carries a session cookie back. This asserts the actual mechanism
  // buyer-app depends on: a bearer token obtained from one response,
  // replayed on a *separate* request with zero cookies involved at all.
