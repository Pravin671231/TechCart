...
- **Shopping cart** (Issues #152/#153 / M4.3–M4.4): `src/features/cart/` + `src/app/cart/page.tsx` (a new static route). `api.ts` injects `getCart` + four mutations against `/api/cart` (all buyer-session-gated); each mutation runs an optimistic `dispatch(cartApi.util.updateQueryData("getCart", undefined, draft => …))` patch in `onQueryStarted`, then replaces the cache with the mutation's own authoritative full-cart response, `.undo()` on error — the workspace's first optimistic-cache usage. `store/api.ts` gained a `"Cart"` `tagType`. `CartContent` mirrors `AccountContent`'s inverted session guard (`session === null` → `router.push("/sign-in?redirect=/cart")`). The shared `AddToCartButton` (product cards + PDP) gates every add on session state — an unauthenticated click routes to `/sign-in?redirect=<pathname>` with no mutation (`FR-CART-020`), and a variant already in the cached cart flips the control to "Go to Cart" (`FR-CART-021`, no per-card request). `PublicProductListItem` gained `defaultVariantId` (backend PR #289) so a card knows which variant to act on. `ProductCard`/`CategoryProductCard` were restructured so the `<Link>` wraps image+text and the button is a sibling (one link per card). `components/layout/CartIndicator.tsx` is the header cart icon: signed-out → `/sign-in?redirect=/cart` link, no badge; signed-in → optimistic `itemCount` badge + hover/focus mini-cart dropdown. `SignInContent` now honours a same-origin `?redirect=` param (protocol-relative / absolute values fall back to `/`); `src/app/sign-in/page.tsx` wraps it in `<Suspense>` (required for `useSearchParams` in this Next version).
...
│   ├── cart.test.tsx                     # renders CartContent: line items, unavailable exclusion, stepper/remove, empty/error, unauth redirect (M4.3)
│   ├── cartIndicator.test.tsx            # renders CartIndicator: signed-out link, itemCount badge, dropdown, optimistic add (M4.3)
│   ├── addToCartButton.test.tsx          # renders AddToCartButton: unauth→sign-in (no mutation), add→"Go to Cart", reversion, disabled (M4.4)
│   ├── auth.test.tsx / account.test.tsx  # sign-in flows (incl. ?redirect=), account profile
│   ├── store/api.test.ts             # api slice: envelope unwrap, error normalization, fail-loud env check
│   └── mocks/{handlers.ts,server.ts}  # shared MSW server; handlers.ts now defaults get-session (signed out) + empty /api/cart
└── src/
    ├── app/{layout.tsx,loading.tsx,page.tsx,globals.css,sign-in/page.tsx,account/page.tsx,cart/page.tsx,category/[slug]/page.tsx,search/page.tsx,products/[slug]/page.tsx}
    ...
    ├── features/cart/{types,api,CartContent,CartLineRow,CartSummary,CartEmpty,CartSkeleton,AddToCartButton}.ts(x)
    ├── features/authentication/{auth,account}/…
