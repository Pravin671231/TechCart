...
│       ├── api.ts                   # productsApi.injectEndpoints — getProducts, exports useGetProductsQuery
│       ├── money.ts                  # formatPrice() — Intl.NumberFormat("en-IN", {style:"currency",currency:"INR"})
│       ├── ProductCard.tsx            # shared card: image+name+price, reused by later listing screens
│       ├── ProductGrid.tsx             # shared grid layout
│       ├── ProductListSkeleton.tsx      # shared loading state
│       ├── ProductListEmpty.tsx          # shared empty state
│       ├── ProductListError.tsx           # shared error state + retry
│       ├── Pagination.tsx                  # shared pagination controls + describeRange()
│       └── SortSelect.tsx                   # shared sort control (native <select>)
└── store/
...
- `src/features/products/` (Issue #72 / M2.15) is the first `injectEndpoints`-owning feature, and is deliberately shared/cross-screen rather than home-specific: `ProductCard`, `ProductGrid`, `Pagination`, `SortSelect`, and the loading/empty/error state components all live here so later listing screens (category, search) reuse them rather than duplicating the toolbar/grid/pagination shape. `src/features/home/HomeContent.tsx` only composes these into the home screen's layout and owns the `page`/`sort` local state (`useState`, no URL-param syncing).
