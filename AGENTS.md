...
- Before marking an issue complete: push the branch and open a PR into `main` (squash-merge, per the convention above); once merged, sync the status sections of `AGENTS.md`, `CLAUDE.md`, `README.md`, and `docs/architecture.md` §10 to reflect the new state in a small follow-up PR (branch protection on `main`, Issue #10, rejects direct pushes — even doc-only syncs go through a PR now), then close the GitHub issue referencing the merge.
