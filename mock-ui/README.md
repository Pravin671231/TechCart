...
| Path      | What's there                                                                                                                                                                          |
| --------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `shared/` | `mock-data.js` (the in-memory dataset), `helpers.js` (all read/query logic over it), plus shared UI chrome (`buyer-header.js`, `admin-nav.js`, `product-grid.js`) reused across pages |
| `buyer/`  | Catalog listing, category, product detail, and search pages                                                                                                                           |
| `admin/`  | Product list/detail/create-edit, category list (+ variant-type editor), brand list                                                                                                    |
...
- **No persistence beyond page reload.** `admin/js/categories.js` and
  `admin/js/brands.js` mutate `window.MOCK` in memory (so create/edit/delete
  actually behave), but `admin/js/product-form.js`'s submit is fully mocked —
  it shows a "saved" banner and writes nothing back, since round-tripping a
  full product/variant edit isn't needed to validate the page layout.
